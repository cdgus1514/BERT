...
# LayerNorm과 Bias에는 Weight Decay를 적용하지 않음 (오버피팅 방지)
for _, v in model.collect_params('.*betal.*gammal.*bias').items():
    v.wd_mult = 0.0
...
for epoch_id in range(num_epochs):
    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in enumerate(train_dataloader):
        if step_num < num_warmup_steps:
            ...
            non_warmpu_step = step_num - num_warmup_steps
            offset = non_warmpu_step / (num_train_steps - num_warmup_steps)
            new_lr = lr - offset * lr
        
        trainer.set_learning_rate(new_lr)

        with mx.qutofrad.record():
            # load data to GPU
        ...
        if(batch_id + 1) % (50) == 0:
            print('[Epoch {} Batch {}/{} loss={:.4f}, lr={:.10f}, acc{:.3f}'.format(epoch_id+1, batch_id+1, len(train_dataloader), setp_loss/log_interval, trainer.learning_rate, metric.get()[1]))
            setp_loss = 0
